-- Premium Server Management System with AI Monitoring, Agent-based Communication, Billing, and Logging

-- 1) Server Metrics (Real-time monitoring data)
...
CREATE TABLE IF NOT EXISTS public.server_ssh_keys (
  ...
  key_type TEXT NOT NULL DEFAULT 'rsa', -- rsa, ed25519
...
CREATE INDEX IF NOT EXISTS idx_server_ssh_keys_server
  ON public.server_ssh_keys(server_id, is_active);

-- 3) Server Agents (Registration and heartbeat tracking)
...
CREATE INDEX IF NOT EXISTS idx_server_agents_server
  ON public.server_agents(server_id);

CREATE INDEX IF NOT EXISTS idx_server_agents_status
  ON public.server_agents(status, last_heartbeat DESC);

-- 4) Server Logs (All operations logged)
CREATE TABLE IF NOT EXISTS public.server_logs (
  ...
  action TEXT NOT NULL, -- 'start', 'stop', 'deploy', 'restart', 'update', 'ssh_connect', etc.
...
CREATE INDEX IF NOT EXISTS idx_server_logs_server
  ON public.server_logs(server_id, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_server_logs_action
  ON public.server_logs(action, status, created_at DESC);

-- 5) Server Billing (Monthly charges: $49/month)
CREATE TABLE IF NOT EXISTS public.server_billing (
  ...
  overage_charges NUMERIC(10,2) DEFAULT 0, -- CPU, RAM overages
  total_amount NUMERIC(10,2) NOT NULL,
  status TEXT NOT NULL DEFAULT 'pending' CHECK (status IN ('pending', 'invoiced', 'paid', 'failed', 'refunded')),
  payment_method TEXT, -- 'card', 'stripe', 'wallet', etc.
...
CREATE INDEX IF NOT EXISTS idx_server_billing_user
  ON public.server_billing(user_id, billing_cycle_start DESC);

CREATE INDEX IF NOT EXISTS idx_server_billing_status
  ON public.server_billing(status, due_date);

-- 6) Server AI Analysis (OpenAI-powered insights)
CREATE TABLE IF NOT EXISTS public.server_ai_analysis (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  server_id UUID NOT NULL REFERENCES public.servers(id) ON DELETE CASCADE,
  analysis_type TEXT NOT NULL, -- 'performance', 'security', 'optimization', 'anomaly'
...
CREATE INDEX IF NOT EXISTS idx_server_ai_analysis_server
  ON public.server_ai_analysis(server_id, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_server_ai_analysis_type
  ON public.server_ai_analysis(analysis_type, analyzed_at DESC);

-- 7) Server SSL Certificates (Enhanced)
...
CREATE INDEX IF NOT EXISTS idx_server_ssl_expires
  ON public.server_ssl_certificates(expires_at);

CREATE INDEX IF NOT EXISTS idx_server_ssl_domain
  ON public.server_ssl_certificates(domain);

-- 8) Server Deployment History (Enhanced)
CREATE TABLE IF NOT EXISTS public.server_deployment_history (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  server_id UUID NOT NULL REFERENCES public.servers(id) ON DELETE CASCADE,
  deployment_type TEXT NOT NULL, -- 'docker', 'nodejs', 'python', 'custom'
  docker_image TEXT,
  env_vars JSONB DEFAULT '{}'::jsonb,
  ports JSONB DEFAULT '[]'::jsonb,
  volumes JSONB DEFAULT '[]'::jsonb,
  resource_limits JSONB DEFAULT '{}'::jsonb, -- { cpu: '1', memory: '2G' }
...
CREATE INDEX IF NOT EXISTS idx_server_deployment_history_server
  ON public.server_deployment_history(server_id, deployed_at DESC);
...
-- RLS Policies: Owner can read/write their server data
