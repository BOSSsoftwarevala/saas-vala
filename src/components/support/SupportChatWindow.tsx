...
export function SupportChatWindow({ channel, messages, members, typingUsers, onSend, onSendFile, onTyping, onSearch, onOpenThread }: Props) {
  const { user } = useAuth();
  ...
  const handleSearch = async () => {
    if (!onSearch) return;
    const q = window.prompt('Search in this channel');
    if (q === null) return;
    await onSearch(q);
  };
    ...
    <div className="flex-1 flex flex-col min-w-0" style={{ background: '#1a1d21' }}>
      ...
      <div className="h-[49px] px-4 flex items-center justify-between border-b flex-shrink-0" style={{ background: '#1a1d21', borderColor: '#383838' }}>
        ...
        <div className="flex items-center gap-[2px]">
          {[
            { icon: Video, label: 'Huddle', action: () => {} },
            { icon: Pin, label: 'Pinned', action: () => {} },
            { icon: Search, label: 'Search', action: handleSearch },
          ].map((item, i) => (
            <Tooltip key={i} delayDuration={0}>
              <TooltipTrigger asChild>
                <button onClick={item.action} className="w-[32px] h-[32px] rounded-[6px] flex items-center justify-center transition-colors hover:bg-white/10" style={{ color: '#b9bbbe' }}>
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {msg.message_type === 'file' ? (
                          <div className="text-[15px] leading-[22px] break-words whitespace-pre-wrap">
                            {msg.file_url ? (
                              <a href={msg.file_url} target="_blank" rel="noreferrer" className="text-[#36c5f0] hover:underline">
                                {msg.file_name || 'Open file'}
                              </a>
                            ) : (
                              <span className="text-white/90">{msg.file_name || 'Shared file'}</span>
                            )}
                          </div>
                        ) : (
