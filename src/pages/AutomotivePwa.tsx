...
const PRODUCTS = [
  { id: 'auto-pwa-1', name: 'Tesla Fleet Management Clone', repo: 'https://github.com/saasvala/tesla-fleet-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'EV fleet management with charging management and vehicle tracking.', features: ['Fleet Management', 'EV Charging Management', 'Vehicle Tracking', 'Maintenance Scheduling', 'Mobile App'] },
  { id: 'auto-pwa-2', name: 'Revv Clone', repo: 'https://github.com/saasvala/revv-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Vehicle rental platform with tracking dashboard and analytics.', features: ['Vehicle Rental', 'Tracking Dashboard', 'Maintenance Scheduling', 'Analytics', 'Mobile App'] },
  { id: 'auto-pwa-3', name: 'Uber EV Management Clone', repo: 'https://github.com/saasvala/uber-ev-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'EV fleet management with route optimization and charging integration.', features: ['EV Fleet Management', 'Route Optimization', 'Charging Station Integration', 'Mobile App', 'Analytics Dashboard'] },
  { id: 'auto-pwa-4', name: 'Ola EV Clone', repo: 'https://github.com/saasvala/ola-ev-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'EV fleet tracking with maintenance scheduling and driver dashboard.', features: ['EV Fleet Tracking', 'Maintenance Scheduling', 'Driver Dashboard', 'Mobile App', 'Payment Integration'] },
  { id: 'auto-pwa-5', name: 'ChargePoint Management Clone', repo: 'https://github.com/saasvala/chargepoint-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Charging station management with fleet analytics and payment gateway.', features: ['Charging Station Management', 'Fleet Analytics', 'Mobile App', 'Maintenance Alerts', 'Payment Gateway'] },
];

const VALID_KEYS = ['AUTO-PWA-2026-001', 'AUTO-PWA-2026-002', 'AUTO-PWA-2026-003', 'AUTO-APK-2026-001'];
const PFX = 'auto-pwa';

export default function AutomotivePwa() {
  ...
  const activate = () => {
    if (VALID_KEYS.includes(keyInput.trim().toUpperCase())) {
      ...
      toast.success('🎉 License activated! All 5 Automotive & EV software demos unlocked.');
    } else toast.error('Invalid license key.');
  };

  const download = () => {
    if (!activated) { setShowKey(true); return; }
    const blob = new Blob([JSON.stringify({ bundle: 'SaaS VALA Automotive & Electric Vehicles Master Copy', version: '2026.1', activated: true, products: PRODUCTS.map(p => ({ name: p.name, repo: p.repo, features: p.features })), generatedAt: new Date().toISOString() }, null, 2)], { type: 'application/json' });
    const u = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = u; a.download = 'saas-vala-automotive-master-copy.json'; a.click(); URL.revokeObjectURL(u);
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <header className="border-b border-border px-4 md:px-8 py-4 flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-primary">SaaS VALA</h1><p className="text-xs text-muted-foreground">Automotive & Electric Vehicles — Offline PWA</p></div>
      ...
      <main className="py-6 space-y-6">
        {!activated && (
          <div className="mx-4 md:mx-8 p-4 rounded-lg border border-primary/30 bg-primary/5 flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-3"><Lock className="h-5 w-5 text-primary" /><div><p className="font-semibold text-sm">Activate to unlock all 5 Automotive & EV Software Demos</p><p className="text-xs text-muted-foreground">Enter license key: AUTO-PWA-2026-001</p></div></div>
        ...
        {activated && (
          <div className="mx-4 md:mx-8 p-4 rounded-lg border border-green-500/30 bg-green-500/5 flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-3"><CheckCircle2 className="h-5 w-5 text-green-500" /><div><p className="font-semibold text-sm">Master Copy Ready — All 5 Automotive Software Unlocked</p><p className="text-xs text-muted-foreground">Download the complete offline bundle</p></div></div>
        ...
        <SectionHeader icon="🚗" title="Automotive & Electric Vehicles" subtitle="Top 5 Automotive & EV Software Clones — Offline Ready." badge="ROW 18" badgeVariant="hot" totalCount={5} />
        <SectionSlider>
          {PRODUCTS.map((p, i) => (
            <div key={p.id} className="min-w-[280px] max-w-[320px] flex-shrink-0 group">
              <Card className="relative overflow-hidden border-border/50 bg-card hover:border-primary/40 transition-all duration-300 hover:scale-[1.05] hover:shadow-[0_0_30px_rgba(249,115,22,0.15)]">
                ...
                <CardContent className="p-4 space-y-3">
                  <Badge variant="outline" className="text-[9px] uppercase tracking-widest text-primary border-primary/30">Automotive</Badge>
      ...
      <Dialog open={showKey} onOpenChange={setShowKey}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-4 pt-2">
            <p className="text-sm text-muted-foreground">Enter your license key to unlock all 5 Automotive & EV software demos offline.</p>
            <Input placeholder="AUTO-PWA-2026-001" value={keyInput} onChange={e => setKeyInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && activate()} className="font-mono text-center tracking-widest" />
