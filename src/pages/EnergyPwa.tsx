...
const PRODUCTS = [
  { id: 'energy-pwa-1', name: 'SmartGrid Clone', repo: 'https://github.com/saasvala/smartgrid-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Energy monitoring with smart grid management and maintenance scheduling.', features: ['Energy Monitoring', 'Smart Grid Management', 'Analytics Dashboard', 'Mobile App', 'Maintenance Scheduling'] },
  { id: 'energy-pwa-2', name: 'UtilityBilling Clone', repo: 'https://github.com/saasvala/utilitybilling-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Billing management with meter reading and payment gateway integration.', features: ['Billing Management', 'Meter Reading', 'Payment Gateway', 'Mobile App', 'Analytics Dashboard'] },
  { id: 'energy-pwa-3', name: 'SolarERP Clone', repo: 'https://github.com/saasvala/solarerpx-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Renewable energy dashboard with project tracking and maintenance alerts.', features: ['Renewable Energy Dashboard', 'Project Tracking', 'Analytics', 'Mobile App', 'Maintenance Alerts'] },
  { id: 'energy-pwa-4', name: 'EnergyManager Clone', repo: 'https://github.com/saasvala/energymanager-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Energy monitoring with consumption analytics and maintenance scheduling.', features: ['Energy Monitoring', 'Consumption Analytics', 'Maintenance Scheduling', 'Mobile App', 'Alerts & Notifications'] },
  { id: 'energy-pwa-5', name: 'GridOps Clone', repo: 'https://github.com/saasvala/gridops-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Smart grid management with energy analytics and fault detection.', features: ['Smart Grid Management', 'Energy Analytics', 'Fault Detection', 'Mobile App', 'Dashboard'] },
];

const VALID_KEYS = ['ENERGY-PWA-2026-001', 'ENERGY-PWA-2026-002', 'ENERGY-PWA-2026-003', 'ENERGY-APK-2026-001'];
const PFX = 'energy-pwa';

export default function EnergyPwa() {
  ...
  const activate = () => {
    if (VALID_KEYS.includes(keyInput.trim().toUpperCase())) {
      ...
      toast.success('🎉 License activated! All 5 Energy & Utilities software demos unlocked.');
    } else toast.error('Invalid license key.');
  };

  const download = () => {
    if (!activated) { setShowKey(true); return; }
    const blob = new Blob([JSON.stringify({ bundle: 'SaaS VALA Energy & Utilities Master Copy', version: '2026.1', activated: true, products: PRODUCTS.map(p => ({ name: p.name, repo: p.repo, features: p.features })), generatedAt: new Date().toISOString() }, null, 2)], { type: 'application/json' });
    const u = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = u; a.download = 'saas-vala-energy-master-copy.json'; a.click(); URL.revokeObjectURL(u);
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <header className="border-b border-border px-4 md:px-8 py-4 flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-primary">SaaS VALA</h1><p className="text-xs text-muted-foreground">Energy & Utilities — Offline PWA</p></div>
      ...
      <main className="py-6 space-y-6">
        {!activated && (
          <div className="mx-4 md:mx-8 p-4 rounded-lg border border-primary/30 bg-primary/5 flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-3"><Lock className="h-5 w-5 text-primary" /><div><p className="font-semibold text-sm">Activate to unlock all 5 Energy & Utilities Software Demos</p><p className="text-xs text-muted-foreground">Enter license key: ENERGY-PWA-2026-001</p></div></div>
        ...
        {activated && (
          <div className="mx-4 md:mx-8 p-4 rounded-lg border border-green-500/30 bg-green-500/5 flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-3"><CheckCircle2 className="h-5 w-5 text-green-500" /><div><p className="font-semibold text-sm">Master Copy Ready — All 5 Energy Software Unlocked</p><p className="text-xs text-muted-foreground">Download the complete offline bundle</p></div></div>
        ...
        <SectionHeader icon="⚡" title="Energy & Utilities" subtitle="Top 5 Energy & Utilities Software Clones — Offline Ready." badge="ROW 20" badgeVariant="hot" totalCount={5} />
        <SectionSlider>
          {PRODUCTS.map((p, i) => (
            <div key={p.id} className="min-w-[280px] max-w-[320px] flex-shrink-0 group">
              <Card className="relative overflow-hidden border-border/50 bg-card hover:border-primary/40 transition-all duration-300 hover:scale-[1.05] hover:shadow-[0_0_30px_rgba(249,115,22,0.15)]">
                ...
                <CardContent className="p-4 space-y-3">
                  <Badge variant="outline" className="text-[9px] uppercase tracking-widest text-primary border-primary/30">Energy & Utilities</Badge>
      ...
      <Dialog open={showKey} onOpenChange={setShowKey}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-4 pt-2">
            <p className="text-sm text-muted-foreground">Enter your license key to unlock all 5 Energy & Utilities software demos offline.</p>
            <Input placeholder="ENERGY-PWA-2026-001" value={keyInput} onChange={e => setKeyInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && activate()} className="font-mono text-center tracking-widest" />
