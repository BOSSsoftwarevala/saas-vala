...
const PRODUCTS = [
  { id: 'telecom-pwa-1', name: 'Asterisk VoIP Clone', repo: 'https://github.com/saasvala/asterisk-voip-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'VoIP management with call routing and customer billing.', features: ['VoIP Management', 'Call Routing', 'Analytics Dashboard', 'Mobile App', 'Customer Billing'] },
  { id: 'telecom-pwa-2', name: 'Bandwidth Manager Clone', repo: 'https://github.com/saasvala/bandwidth-manager-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Internet monitoring with usage analytics and payment integration.', features: ['Internet Monitoring', 'Usage Analytics', 'Customer Dashboard', 'Payment Integration', 'Mobile App'] },
  { id: 'telecom-pwa-3', name: 'MikroTik ISP Clone', repo: 'https://github.com/saasvala/mikrotik-isp-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Network management with user billing and analytics.', features: ['Network Management', 'User Billing', 'Analytics', 'Mobile App', 'Alerts & Notifications'] },
  { id: 'telecom-pwa-4', name: 'Cloud PBX Clone', repo: 'https://github.com/saasvala/cloudpbx-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'VoIP management with call analytics and customer billing.', features: ['VoIP Management', 'Call Analytics', 'Customer Billing', 'Mobile App', 'Dashboard'] },
  { id: 'telecom-pwa-5', name: 'ISP Billing Clone', repo: 'https://github.com/saasvala/isp-billing-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Customer billing with payment gateway and usage analytics.', features: ['Customer Billing', 'Payment Gateway', 'Usage Analytics', 'Mobile App', 'Service Dashboard'] },
];

const PFX = 'telecom-pwa';

export default function TelecomPwa() {
  ...
  const activate = async () => {
    ...
    if (result.valid) {
      ...
      toast.success('🎉 License activated! All 5 Telecom & Internet software demos unlocked.');
    } else toast.error(result.error || 'Invalid license key.');
  };

  const download = () => {
    if (!activated) { setShowKey(true); return; }
    const blob = new Blob([JSON.stringify({ bundle: 'SaaS VALA Telecom & Internet Services Master Copy', version: '2026.1', activated: true, products: PRODUCTS.map(p => ({ name: p.name, repo: p.repo, features: p.features })), generatedAt: new Date().toISOString() }, null, 2)], { type: 'application/json' });
    const u = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = u; a.download = 'saas-vala-telecom-master-copy.json'; a.click(); URL.revokeObjectURL(u);
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <header className="border-b border-border px-4 md:px-8 py-4 flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-primary">SaaS VALA</h1><p className="text-xs text-muted-foreground">Telecom & Internet Services — Offline PWA</p></div>
      ...
      <main className="py-6 space-y-6">
        {!activated && (
          <div className="mx-4 md:mx-8 p-4 rounded-lg border border-primary/30 bg-primary/5 flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-3"><Lock className="h-5 w-5 text-primary" /><div><p className="font-semibold text-sm">Activate to unlock all 5 Telecom & Internet Software Demos</p><p className="text-xs text-muted-foreground">Enter license key: TELECOM-PWA-2026-001</p></div></div>
        ...
        {activated && (
          <div className="mx-4 md:mx-8 p-4 rounded-lg border border-green-500/30 bg-green-500/5 flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-3"><CheckCircle2 className="h-5 w-5 text-green-500" /><div><p className="font-semibold text-sm">Master Copy Ready — All 5 Telecom Software Unlocked</p><p className="text-xs text-muted-foreground">Download the complete offline bundle</p></div></div>
        ...
        <SectionHeader icon="📡" title="Telecom & Internet Services" subtitle="Top 5 Telecom & Internet Software Clones — Offline Ready." badge="ROW 21" badgeVariant="hot" totalCount={5} />
        <SectionSlider>
          {PRODUCTS.map((p, i) => (
            <div key={p.id} className="min-w-[280px] max-w-[320px] flex-shrink-0 group">
              <Card className="relative overflow-hidden border-border/50 bg-card hover:border-primary/40 transition-all duration-300 hover:scale-[1.05] hover:shadow-[0_0_30px_rgba(249,115,22,0.15)]">
                ...
                <CardContent className="p-4 space-y-3">
                  <Badge variant="outline" className="text-[9px] uppercase tracking-widest text-primary border-primary/30">Telecom & Internet</Badge>
      ...
      <Dialog open={showKey} onOpenChange={setShowKey}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-4 pt-2">
            <p className="text-sm text-muted-foreground">Enter your license key to unlock all 5 Telecom & Internet software demos offline.</p>
            <Input placeholder="TELECOM-PWA-2026-001" value={keyInput} onChange={e => setKeyInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && activate()} className="font-mono text-center tracking-widest" />
