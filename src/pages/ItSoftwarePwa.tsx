...
const PRODUCTS = [
  { id: 'it-pwa-1', name: 'Jira Clone', repo: 'https://github.com/saasvala/jira-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Project management with issue tracking and collaboration tools.', features: ['Project Management', 'Issue Tracking', 'Analytics Dashboard', 'Mobile App', 'Collaboration Tools'] },
  { id: 'it-pwa-2', name: 'GitLab Clone', repo: 'https://github.com/saasvala/gitlab-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'CI/CD tools with repository management and project dashboard.', features: ['CI/CD Tools', 'Repository Management', 'Project Dashboard', 'Mobile App', 'Analytics'] },
  { id: 'it-pwa-3', name: 'Asana Clone', repo: 'https://github.com/saasvala/asana-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Task management with project tracking and team collaboration.', features: ['Task Management', 'Project Tracking', 'Team Collaboration', 'Mobile App', 'Analytics Dashboard'] },
  { id: 'it-pwa-4', name: 'Trello Clone', repo: 'https://github.com/saasvala/trello-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Kanban boards with task management and collaboration tools.', features: ['Kanban Boards', 'Task Management', 'Collaboration Tools', 'Mobile App', 'Dashboard Analytics'] },
...
export default function ItSoftwarePwa() {
  ...
  const download = () => {
    ...
    const blob = new Blob([JSON.stringify({ bundle: 'SaaS VALA IT & Software Master Copy', version: '2026.1', activated: true, products: PRODUCTS.map(p => ({ name: p.name, repo: p.repo, features: p.features })), generatedAt: new Date().toISOString() }, null, 2)], { type: 'application/json' });
    const u = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = u; a.download = 'saas-vala-it-software-master-copy.json'; a.click(); URL.revokeObjectURL(u);
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <header className="border-b border-border px-4 md:px-8 py-4 flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-primary">SaaS VALA</h1><p className="text-xs text-muted-foreground">Information Technology & Software — Offline PWA</p></div>
      ...
      <main className="py-6 space-y-6">
        {!activated && (
          <div className="mx-4 md:mx-8 p-4 rounded-lg border border-primary/30 bg-primary/5 flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-3"><Lock className="h-5 w-5 text-primary" /><div><p className="font-semibold text-sm">Activate to unlock all 5 IT & Software Demos</p><p className="text-xs text-muted-foreground">Enter license key: IT-PWA-2026-001</p></div></div>
        ...
        {activated && (
          <div className="mx-4 md:mx-8 p-4 rounded-lg border border-green-500/30 bg-green-500/5 flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-3"><CheckCircle2 className="h-5 w-5 text-green-500" /><div><p className="font-semibold text-sm">Master Copy Ready — All 5 IT Software Unlocked</p><p className="text-xs text-muted-foreground">Download the complete offline bundle</p></div></div>
        ...
        <SectionHeader icon="💻" title="Information Technology & Software" subtitle="Top 5 IT & Software Clones — Offline Ready." badge="ROW 22" badgeVariant="hot" totalCount={5} />
        <SectionSlider>
          {PRODUCTS.map((p, i) => (
            <div key={p.id} className="min-w-[280px] max-w-[320px] flex-shrink-0 group">
              <Card className="relative overflow-hidden border-border/50 bg-card hover:border-primary/40 transition-all duration-300 hover:scale-[1.05] hover:shadow-[0_0_30px_rgba(249,115,22,0.15)]">
                ...
                <CardContent className="p-4 space-y-3">
                  <Badge variant="outline" className="text-[9px] uppercase tracking-widest text-primary border-primary/30">IT & Software</Badge>
      ...
      <Dialog open={showKey} onOpenChange={setShowKey}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-4 pt-2">
            <p className="text-sm text-muted-foreground">Enter your license key to unlock all 5 IT & Software demos offline.</p>
            <Input placeholder="IT-PWA-2026-001" value={keyInput} onChange={e => setKeyInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && activate()} className="font-mono text-center tracking-widest" />
