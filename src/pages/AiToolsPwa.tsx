...
const PRODUCTS = [
  { id: 'ai-apk-1', name: 'ChatGPT AI Platform Clone', demoFolder: 'chatgpt-ai', repo: 'https://github.com/saasvala/chatgpt-ai-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'AI conversational platform with text generation and chatbot.', features: ['AI Text Generation', 'Image Generation', 'Workflow Automation', 'API Integration', 'AI Chatbot', 'Data Analysis', 'AI Dashboard'] },
  { id: 'ai-apk-2', name: 'Midjourney Image Generator Clone', demoFolder: 'midjourney-image', repo: 'https://github.com/saasvala/midjourney-image-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'AI image generation platform with creative workflows.', features: ['AI Text Generation', 'Image Generation', 'Workflow Automation', 'API Integration', 'AI Chatbot', 'Data Analysis', 'AI Dashboard'] },
  { id: 'ai-apk-3', name: 'Jasper AI Writer Clone', demoFolder: 'jasper-ai-writer', repo: 'https://github.com/saasvala/jasper-ai-writer-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'AI content writing platform with templates and automation.', features: ['AI Text Generation', 'Image Generation', 'Workflow Automation', 'API Integration', 'AI Chatbot', 'Data Analysis', 'AI Dashboard'] },
  { id: 'ai-apk-4', name: 'Zapier Automation Clone', demoFolder: 'zapier-automation', repo: 'https://github.com/saasvala/zapier-automation-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Workflow automation platform connecting apps and services.', features: ['AI Text Generation', 'Image Generation', 'Workflow Automation', 'API Integration', 'AI Chatbot', 'Data Analysis', 'AI Dashboard'] },
  { id: 'ai-apk-5', name: 'Notion AI Workspace Clone', demoFolder: 'notion-ai', repo: 'https://github.com/saasvala/notion-ai-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'AI-powered workspace with docs, databases and automation.', features: ['AI Text Generation', 'Image Generation', 'Workflow Automation', 'API Integration', 'AI Chatbot', 'Data Analysis', 'AI Dashboard'] },
];
...
export default function AiToolsPwa() {
  ...
  const activate = async () => {
    ...
    if (result.valid) {
      ...
      toast.success(`🎉 License activated! Valid for ${daysLeft} days. All 5 AI & Automation software demos unlocked.`);
  ...
  const handleMasterDownload = () => {
    ...
    const blob = new Blob([JSON.stringify({ bundle: 'SaaS VALA AI & Automation Tools Master Copy', version: '2026.1', row: 16, license: lic, products: PRODUCTS.map(p => ({ name: p.name, repo: p.repo, features: p.features, demoFolder: p.demoFolder })), generatedAt: new Date().toISOString() }, null, 2)], { type: 'application/json' });
    const u = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = u; a.download = 'saas-vala-ai-automation-master-copy.json'; a.click(); URL.revokeObjectURL(u);
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <header className="border-b border-border px-4 md:px-8 py-4 flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-primary">SaaS VALA</h1><p className="text-xs text-muted-foreground">AI & Automation Tools — Offline PWA</p></div>
      ...
      <main className="py-6 space-y-6">
        {!licenseState.valid && (
          <div className={cn("mx-4 md:mx-8 p-4 rounded-lg border flex items-center justify-between gap-4 flex-wrap", licenseState.expired ? "border-destructive/30 bg-destructive/5" : "border-primary/30 bg-primary/5")}>
            <div className="flex items-center gap-3">{licenseState.expired ? <AlertTriangle className="h-5 w-5 text-destructive" /> : <Lock className="h-5 w-5 text-primary" />}<div><p className="font-semibold text-sm">{licenseState.expired ? 'License Expired — Enter a new key to continue' : 'Activate to unlock all 5 AI & Automation Software Demos'}</p><p className="text-xs text-muted-foreground">Enter license key: AI-APK-2026-001</p></div></div>
        ...
        {licenseState.valid && (
          <div className="mx-4 md:mx-8 p-4 rounded-lg border border-green-500/30 bg-green-500/5 flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-3"><CheckCircle2 className="h-5 w-5 text-green-500" /><div><p className="font-semibold text-sm">Master Copy Ready — All 5 AI & Automation Software Unlocked</p><p className="text-xs text-muted-foreground">License valid for {licenseState.daysLeft} days · Download the complete offline bundle</p></div></div>
        ...
        <SectionHeader icon="🤖" title="AI & Automation Tools" subtitle="Top 5 AI & Automation Software Clones — Offline Ready." badge="ROW 16" badgeVariant="hot" totalCount={5} />
        <SectionSlider>
          {PRODUCTS.map((p, i) => (
            <div key={p.id} className="min-w-[280px] max-w-[320px] flex-shrink-0 group">
              <Card className="relative overflow-hidden border-border/50 bg-card hover:border-primary/40 transition-all duration-300 hover:scale-[1.05] hover:shadow-[0_0_30px_rgba(249,115,22,0.15)]">
                ...
                <CardContent className="p-4 space-y-3">
                  <Badge variant="outline" className="text-[9px] uppercase tracking-widest text-primary border-primary/30">AI Tools</Badge>
      ...
      <Dialog open={showKey} onOpenChange={setShowKey}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-4 pt-2">
            <p className="text-sm text-muted-foreground">Enter your license key to unlock all 5 AI & Automation software demos offline for {LICENSE_DAYS} days.</p>
            <Input placeholder="AI-APK-2026-001" value={keyInput} onChange={e => setKeyInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && activate()} className="font-mono text-center tracking-widest" />
