...
const PRODUCTS = [
  {
    id: 'ecom-1', name: 'Amazon Marketplace Clone',
    repo: 'https://github.com/saasvala/amazon-marketplace-clone-software',
    price: 5, old_price: 10, rating: 4.9,
    description: 'Multi-vendor marketplace with product catalog, orders, and seller dashboard.',
    features: ['Multi-Vendor Marketplace', 'Product Catalog', 'Order Management', 'Payment Gateway', 'Customer Reviews', 'Seller Dashboard', 'Inventory Management'],
    demoFolder: 'amazon-marketplace',
  },
  {
    id: 'ecom-2', name: 'Shopify Store Clone',
    repo: 'https://github.com/saasvala/shopify-store-clone-software',
    price: 5, old_price: 10, rating: 4.9,
    description: 'Complete e-commerce storefront builder with payment and inventory tools.',
    features: ['Multi-Vendor Marketplace', 'Product Catalog', 'Order Management', 'Payment Gateway', 'Customer Reviews', 'Seller Dashboard', 'Inventory Management'],
    demoFolder: 'shopify-store',
  },
  {
    id: 'ecom-3', name: 'Flipkart Marketplace Clone',
    repo: 'https://github.com/saasvala/flipkart-marketplace-clone-software',
    price: 5, old_price: 10, rating: 4.9,
    description: 'Indian e-commerce marketplace with vendor panel and order tracking.',
    features: ['Multi-Vendor Marketplace', 'Product Catalog', 'Order Management', 'Payment Gateway', 'Customer Reviews', 'Seller Dashboard', 'Inventory Management'],
    demoFolder: 'flipkart-marketplace',
  },
  {
    id: 'ecom-4', name: 'eBay Marketplace Clone',
    repo: 'https://github.com/saasvala/ebay-marketplace-clone-software',
    price: 5, old_price: 10, rating: 4.9,
    description: 'Auction and buy-it-now marketplace with seller tools and reviews.',
    features: ['Multi-Vendor Marketplace', 'Product Catalog', 'Order Management', 'Payment Gateway', 'Customer Reviews', 'Seller Dashboard', 'Inventory Management'],
    demoFolder: 'ebay-marketplace',
  },
  {
    id: 'ecom-5', name: 'Alibaba B2B Marketplace Clone',
    repo: 'https://github.com/saasvala/alibaba-b2b-clone-software',
    price: 5, old_price: 10, rating: 4.9,
    description: 'B2B wholesale marketplace connecting manufacturers and global buyers.',
    features: ['Multi-Vendor Marketplace', 'Product Catalog', 'Order Management', 'Payment Gateway', 'Customer Reviews', 'Seller Dashboard', 'Inventory Management'],
    demoFolder: 'alibaba-b2b',
  },
];
...
export default function EcomPwa() {
  ...
  const handleMasterDownload = () => {
    ...
    const blob = new Blob([JSON.stringify({ bundle: 'SaaS VALA E-Commerce Master Copy', version: '2026.1', license: getLicense(), products: PRODUCTS.map(p => ({ name: p.name, repo: p.repo, demoFolder: p.demoFolder, features: p.features })), generatedAt: new Date().toISOString() }, null, 2)], { type: 'application/json' });
    const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'saas-vala-ecommerce-master-copy.json'; a.click(); URL.revokeObjectURL(url);
    ...
    <div className="min-h-screen bg-background text-foreground">
      <header className="border-b border-border px-4 md:px-8 py-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">E-Commerce & Marketplaces — Offline APK</p>
        </div>
      ...
      <main className="py-6 space-y-6">
        ...
        {!licStatus.valid && !licStatus.expired && (
          <div className="mx-4 md:mx-8 p-4 rounded-lg border border-primary/30 bg-primary/5 flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-3"><Lock className="h-5 w-5 text-primary" /><div><p className="font-semibold text-sm">Activate to unlock all 5 E-Commerce Software Demos</p><p className="text-xs text-muted-foreground">Enter license key: ECOM-APK-2026-001</p></div></div>
      ...
      <Dialog open={showActivation} onOpenChange={setShowActivation}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-4 pt-2">
            <p className="text-sm text-muted-foreground">Enter your license key to unlock all 5 E-Commerce demos for 30 days.</p>
            <Input placeholder="ECOM-APK-2026-001" value={keyInput} onChange={e => setKeyInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleActivate()} className="font-mono text-center tracking-widest" />
