...
const PRODUCTS = [
  { id: 'ecom-pwa-1', name: 'Shopify Clone', repo: 'https://github.com/saasvala/shopify-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Complete e-commerce platform for building online stores with payment integration.', features: ['Product Catalog', 'Shopping Cart', 'Payment Gateway', 'Order Management', 'User Dashboard'] },
  { id: 'ecom-pwa-2', name: 'Magento Clone', repo: 'https://github.com/saasvala/magento-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Enterprise e-commerce platform with multi-vendor support and order tracking.', features: ['Product Management', 'Multi-Vendor', 'Order Tracking', 'Payment Integration', 'Admin Panel'] },
  { id: 'ecom-pwa-3', name: 'WooCommerce Clone', repo: 'https://github.com/saasvala/woocommerce-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Flexible e-commerce solution with product listings, cart, and coupon system.', features: ['Product Listing', 'Shopping Cart', 'Checkout System', 'Coupons & Discounts', 'Mobile App'] },
  { id: 'ecom-pwa-4', name: 'BigCommerce Clone', repo: 'https://github.com/saasvala/bigcommerce-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Scalable storefront platform with analytics and order management.', features: ['Storefront', 'Product Categories', 'Payment Gateway', 'Order Management', 'Analytics Dashboard'] },
  { id: 'ecom-pwa-5', name: 'Flipkart Clone', repo: 'https://github.com/saasvala/flipkart-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Indian e-commerce marketplace with vendor panel and order tracking.', features: ['Product Catalog', 'Vendor Panel', 'Payment Integration', 'Order Tracking', 'User Reviews'] },
];

const VALID_KEYS = ['ECOM-PWA-2026-001', 'ECOM-PWA-2026-002', 'ECOM-PWA-2026-003', 'ECOM-APK-2026-001'];
const PFX = 'ecom-pwa';

export default function EcomPwa() {
  ...
  const activate = () => {
    if (VALID_KEYS.includes(keyInput.trim().toUpperCase())) {
      ...
      toast.success('🎉 License activated! All 5 E-Commerce software demos unlocked.');
    } else toast.error('Invalid license key.');
  };

  const download = () => {
    if (!activated) { setShowKey(true); return; }
    const blob = new Blob([JSON.stringify({ bundle: 'SaaS VALA E-Commerce Master Copy', version: '2026.1', activated: true, products: PRODUCTS.map(p => ({ name: p.name, repo: p.repo, features: p.features })), generatedAt: new Date().toISOString() }, null, 2)], { type: 'application/json' });
    const u = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = u; a.download = 'saas-vala-ecommerce-master-copy.json'; a.click(); URL.revokeObjectURL(u);
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <header className="border-b border-border px-4 md:px-8 py-4 flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-primary">SaaS VALA</h1><p className="text-xs text-muted-foreground">E-Commerce & Marketplaces — Offline PWA</p></div>
      ...
      <main className="py-6 space-y-6">
        {!activated && (
          <div className="mx-4 md:mx-8 p-4 rounded-lg border border-primary/30 bg-primary/5 flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-3"><Lock className="h-5 w-5 text-primary" /><div><p className="font-semibold text-sm">Activate to unlock all 5 E-Commerce Software Demos</p><p className="text-xs text-muted-foreground">Enter license key: ECOM-PWA-2026-001</p></div></div>
        ...
        {activated && (
          <div className="mx-4 md:mx-8 p-4 rounded-lg border border-green-500/30 bg-green-500/5 flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-3"><CheckCircle2 className="h-5 w-5 text-green-500" /><div><p className="font-semibold text-sm">Master Copy Ready — All 5 E-Commerce Software Unlocked</p><p className="text-xs text-muted-foreground">Download the complete offline bundle</p></div></div>
        ...
        <SectionHeader icon="🛒" title="E-Commerce & Online Marketplaces" subtitle="Top 5 E-Commerce Software Clones — Offline Ready." badge="ROW 08" badgeVariant="hot" totalCount={5} />
        <SectionSlider>
          {PRODUCTS.map((p, i) => (
            <div key={p.id} className="min-w-[280px] max-w-[320px] flex-shrink-0 group">
              <Card className="relative overflow-hidden border-border/50 bg-card hover:border-primary/40 transition-all duration-300 hover:scale-[1.05] hover:shadow-[0_0_30px_rgba(249,115,22,0.15)]">
                ...
                <CardContent className="p-4 space-y-3">
                  <Badge variant="outline" className="text-[9px] uppercase tracking-widest text-primary border-primary/30">E-Commerce</Badge>
      ...
      <Dialog open={showKey} onOpenChange={setShowKey}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-4 pt-2">
            <p className="text-sm text-muted-foreground">Enter your license key to unlock all 5 E-Commerce software demos offline.</p>
            <Input placeholder="ECOM-PWA-2026-001" value={keyInput} onChange={e => setKeyInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && activate()} className="font-mono text-center tracking-widest" />
