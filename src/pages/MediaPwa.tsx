...
const PRODUCTS = [
  { id: 'media-apk-1', name: 'Netflix Streaming Platform Clone', demoFolder: 'netflix-streaming', repo: 'https://github.com/saasvala/netflix-streaming-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Video streaming platform with subscriptions and recommendation engine.', features: ['Video Streaming', 'User Subscriptions', 'Content Library', 'Watch History', 'Video Upload', 'Recommendation Engine', 'Admin Dashboard'] },
  { id: 'media-apk-2', name: 'YouTube Video Platform Clone', demoFolder: 'youtube-video', repo: 'https://github.com/saasvala/youtube-video-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Video sharing platform with uploads, subscriptions and watch history.', features: ['Video Streaming', 'User Subscriptions', 'Content Library', 'Watch History', 'Video Upload', 'Recommendation Engine', 'Admin Dashboard'] },
  { id: 'media-apk-3', name: 'Spotify Music Streaming Clone', demoFolder: 'spotify-music', repo: 'https://github.com/saasvala/spotify-music-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Music streaming platform with playlists and recommendation engine.', features: ['Video Streaming', 'User Subscriptions', 'Content Library', 'Watch History', 'Video Upload', 'Recommendation Engine', 'Admin Dashboard'] },
  { id: 'media-apk-4', name: 'Disney+ Streaming Clone', demoFolder: 'disneyplus-streaming', repo: 'https://github.com/saasvala/disneyplus-streaming-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Premium streaming service with content library and user profiles.', features: ['Video Streaming', 'User Subscriptions', 'Content Library', 'Watch History', 'Video Upload', 'Recommendation Engine', 'Admin Dashboard'] },
...
export default function MediaPwa() {
  ...
  const handleMasterDownload = () => {
    ...
    const blob = new Blob([JSON.stringify({ bundle: 'SaaS VALA Media, Streaming & Entertainment Platforms Master Copy', version: '2026.1', row: 14, license: lic, products: PRODUCTS.map(p => ({ name: p.name, repo: p.repo, features: p.features, demoFolder: p.demoFolder })), generatedAt: new Date().toISOString() }, null, 2)], { type: 'application/json' });
    const u = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = u; a.download = 'saas-vala-media-streaming-master-copy.json'; a.click(); URL.revokeObjectURL(u);
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <header className="border-b border-border px-4 md:px-8 py-4 flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-primary">SaaS VALA</h1><p className="text-xs text-muted-foreground">Media, Streaming & Entertainment — Offline PWA</p></div>
      ...
      <main className="py-6 space-y-6">
        {!licenseState.valid && (
          <div className={cn("mx-4 md:mx-8 p-4 rounded-lg border flex items-center justify-between gap-4 flex-wrap", licenseState.expired ? "border-destructive/30 bg-destructive/5" : "border-primary/30 bg-primary/5")}>
            <div className="flex items-center gap-3">{licenseState.expired ? <AlertTriangle className="h-5 w-5 text-destructive" /> : <Lock className="h-5 w-5 text-primary" />}<div><p className="font-semibold text-sm">{licenseState.expired ? 'License Expired — Enter a new key to continue' : 'Activate to unlock all 5 Media & Streaming Software Demos'}</p><p className="text-xs text-muted-foreground">Enter license key: MEDIA-APK-2026-001</p></div></div>
        ...
        {licenseState.valid && (
          <div className="mx-4 md:mx-8 p-4 rounded-lg border border-green-500/30 bg-green-500/5 flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-3"><CheckCircle2 className="h-5 w-5 text-green-500" /><div><p className="font-semibold text-sm">Master Copy Ready — All 5 Media & Streaming Software Unlocked</p><p className="text-xs text-muted-foreground">License valid for {licenseState.daysLeft} days · Download the complete offline bundle</p></div></div>
        ...
        <SectionHeader icon="🎬" title="Media, Streaming & Entertainment Platforms" subtitle="Top 5 Media & Streaming Software Clones — Offline Ready." badge="ROW 14" badgeVariant="hot" totalCount={5} />
        <SectionSlider>
          {PRODUCTS.map((p, i) => (
            <div key={p.id} className="min-w-[280px] max-w-[320px] flex-shrink-0 group">
              <Card className="relative overflow-hidden border-border/50 bg-card hover:border-primary/40 transition-all duration-300 hover:scale-[1.05] hover:shadow-[0_0_30px_rgba(249,115,22,0.15)]">
                ...
                <CardContent className="p-4 space-y-3">
                  <Badge variant="outline" className="text-[9px] uppercase tracking-widest text-primary border-primary/30">Media</Badge>
      ...
      <Dialog open={showKey} onOpenChange={setShowKey}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-4 pt-2">
            <p className="text-sm text-muted-foreground">Enter your license key to unlock all 5 Media & Streaming software demos offline for {LICENSE_DAYS} days.</p>
            <Input placeholder="MEDIA-APK-2026-001" value={keyInput} onChange={e => setKeyInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && activate()} className="font-mono text-center tracking-widest" />
