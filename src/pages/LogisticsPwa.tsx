...
const PRODUCTS = [
  { id: 'log-pwa-1', name: 'ShipStation Clone', repo: 'https://github.com/saasvala/shipstation-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Shipment tracking and order processing with warehouse management.', features: ['Shipment Tracking', 'Inventory Management', 'Order Processing', 'Warehouse Management', 'Analytics Dashboard'] },
  { id: 'log-pwa-2', name: 'Track-POD Clone', repo: 'https://github.com/saasvala/trackpod-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Fleet monitoring with proof of delivery and shipment tracking.', features: ['Fleet Monitoring', 'Shipment Tracking', 'Proof of Delivery', 'Mobile App', 'Analytics Dashboard'] },
  { id: 'log-pwa-3', name: 'LogiNext Clone', repo: 'https://github.com/saasvala/loginext-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Fleet management with route optimization and order tracking.', features: ['Fleet Management', 'Route Optimization', 'Order Tracking', 'Mobile App', 'Analytics Dashboard'] },
  { id: 'log-pwa-4', name: 'Flexport Clone', repo: 'https://github.com/saasvala/flexport-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Global shipment tracking with inventory and warehouse dashboard.', features: ['Shipment Tracking', 'Inventory Management', 'Warehouse Dashboard', 'Analytics', 'Mobile App'] },
  { id: 'log-pwa-5', name: 'Fretron Clone', repo: 'https://github.com/saasvala/fretron-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Fleet monitoring with order processing and inventory tracking.', features: ['Fleet Monitoring', 'Order Processing', 'Inventory Tracking', 'Mobile App', 'Analytics Dashboard'] },
];

const PFX = 'log-pwa';

export default function LogisticsPwa() {
  ...
  const activate = async () => {
    ...
    if (result.valid) {
      ...
      toast.success('🎉 License activated! All 5 Logistics & Supply Chain software demos unlocked.');
    } else toast.error(result.error || 'Invalid license key.');
  };

  const download = () => {
    if (!activated) { setShowKey(true); return; }
    const blob = new Blob([JSON.stringify({ bundle: 'SaaS VALA Logistics & Supply Chain Master Copy', version: '2026.1', activated: true, products: PRODUCTS.map(p => ({ name: p.name, repo: p.repo, features: p.features })), generatedAt: new Date().toISOString() }, null, 2)], { type: 'application/json' });
    const u = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = u; a.download = 'saas-vala-logistics-master-copy.json'; a.click(); URL.revokeObjectURL(u);
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <header className="border-b border-border px-4 md:px-8 py-4 flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-primary">SaaS VALA</h1><p className="text-xs text-muted-foreground">Logistics & Supply Chain — Offline PWA</p></div>
      ...
      <main className="py-6 space-y-6">
        {!activated && (
          <div className="mx-4 md:mx-8 p-4 rounded-lg border border-primary/30 bg-primary/5 flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-3"><Lock className="h-5 w-5 text-primary" /><div><p className="font-semibold text-sm">Activate to unlock all 5 Logistics & Supply Chain Software Demos</p><p className="text-xs text-muted-foreground">Enter license key: LOGISTICS-PWA-2026-001</p></div></div>
        ...
        {activated && (
          <div className="mx-4 md:mx-8 p-4 rounded-lg border border-green-500/30 bg-green-500/5 flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-3"><CheckCircle2 className="h-5 w-5 text-green-500" /><div><p className="font-semibold text-sm">Master Copy Ready — All 5 Logistics Software Unlocked</p><p className="text-xs text-muted-foreground">Download the complete offline bundle</p></div></div>
        ...
        <SectionHeader icon="📦" title="Logistics & Supply Chain" subtitle="Top 5 Logistics & Supply Chain Software Clones — Offline Ready." badge="ROW 13" badgeVariant="hot" totalCount={5} />
        <SectionSlider>
          {PRODUCTS.map((p, i) => (
            <div key={p.id} className="min-w-[280px] max-w-[320px] flex-shrink-0 group">
              <Card className="relative overflow-hidden border-border/50 bg-card hover:border-primary/40 transition-all duration-300 hover:scale-[1.05] hover:shadow-[0_0_30px_rgba(249,115,22,0.15)]">
                ...
                <CardContent className="p-4 space-y-3">
                  <Badge variant="outline" className="text-[9px] uppercase tracking-widest text-primary border-primary/30">Logistics</Badge>
      ...
      <Dialog open={showKey} onOpenChange={setShowKey}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-4 pt-2">
            <p className="text-sm text-muted-foreground">Enter your license key to unlock all 5 Logistics & Supply Chain software demos offline.</p>
            <Input placeholder="LOGISTICS-PWA-2026-001" value={keyInput} onChange={e => setKeyInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && activate()} className="font-mono text-center tracking-widest" />
