      ...
      <div className="rounded-2xl overflow-hidden border border-border/50 shadow-xl h-full flex flex-col" style={{ background: 'hsl(var(--card))' }}>
        ...
        <div className="relative h-[140px] overflow-hidden">
          ...
          <Button
            ...
            size="icon"
            className={cn(
              'absolute top-3 left-3 h-8 w-8 rounded-full backdrop-blur-sm',
              wishlisted ? 'bg-pink-500/20 text-pink-400' : 'bg-background/50 text-foreground hover:text-pink-400'
            )}
        ...
        <div className="flex flex-col flex-1 p-4 gap-2">
          <h4 className="font-black text-[14px] text-foreground uppercase leading-tight line-clamp-2">
            {product.title}
          </h4>
          ...
          <div className="flex flex-wrap gap-1 mt-1">
            {product.features.slice(0, 4).map((f, i) => (
              <span key={i} className="inline-flex items-center gap-1 text-[10px] border border-border/50 text-foreground px-1.5 py-0.5 rounded" style={{ background: 'rgba(255,255,255,0.04)' }}>
                <Box style={{ width: 10, height: 10, color: '#9ca3af' }} />
                {f}
              </span>
            ))}
          </div>
          ...
          <div className="flex items-center gap-2 mt-2">
            ...
            <span className="text-[10px] font-black px-1.5 py-0.5 rounded-full" style={{ background: 'rgba(239,68,68,0.2)', color: '#f87171', border: '1px solid rgba(239,68,68,0.3)' }}>
              {discount}% OFF
            </span>
            ...
            {isPipeline ? (
              ...
              <>
                ...
                <Button
                  size="sm"
                  variant="outline"
                  className="h-9 w-9 p-0 rounded-xl border-border hover:border-emerald-400/50 hover:text-emerald-400"
                  onClick={handleDownload}
                  title="Download / Clone"
                >
