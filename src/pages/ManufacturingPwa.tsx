...
const PRODUCTS = [
  { id: 'manuf-pwa-1', name: 'Odoo Manufacturing Clone', repo: 'https://github.com/saasvala/odoo-manufacturing-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Production planning with inventory management and ERP integration.', features: ['Production Planning', 'Inventory Management', 'ERP Integration', 'Quality Control', 'Machine Monitoring'] },
  { id: 'manuf-pwa-2', name: 'SAP ERP Clone', repo: 'https://github.com/saasvala/sap-erp-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Enterprise ERP with production scheduling and workforce management.', features: ['ERP Integration', 'Inventory Management', 'Production Scheduling', 'Workforce Management', 'Analytics Dashboard'] },
  { id: 'manuf-pwa-3', name: 'Oracle NetSuite Clone', repo: 'https://github.com/saasvala/netsuite-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Cloud production management with inventory control and order processing.', features: ['Production Management', 'Inventory Control', 'Order Processing', 'ERP Integration', 'Mobile Dashboard'] },
  { id: 'manuf-pwa-4', name: 'Fishbowl Manufacturing Clone', repo: 'https://github.com/saasvala/fishbowl-manufacturing-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Inventory and production scheduling with order management.', features: ['Inventory Management', 'Production Scheduling', 'Order Management', 'ERP Integration', 'Mobile App'] },
...
export default function ManufacturingPwa() {
  ...
  const download = () => {
    ...
    const blob = new Blob([JSON.stringify({ bundle: 'SaaS VALA Manufacturing & Industrial Production Master Copy', version: '2026.1', activated: true, products: PRODUCTS.map(p => ({ name: p.name, repo: p.repo, features: p.features })), generatedAt: new Date().toISOString() }, null, 2)], { type: 'application/json' });
    const u = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = u; a.download = 'saas-vala-manufacturing-master-copy.json'; a.click(); URL.revokeObjectURL(u);
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <header className="border-b border-border px-4 md:px-8 py-4 flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-primary">SaaS VALA</h1><p className="text-xs text-muted-foreground">Manufacturing & Industrial Production — Offline PWA</p></div>
      ...
      <main className="py-6 space-y-6">
        {!activated && (
          <div className="mx-4 md:mx-8 p-4 rounded-lg border border-primary/30 bg-primary/5 flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-3"><Lock className="h-5 w-5 text-primary" /><div><p className="font-semibold text-sm">Activate to unlock all 5 Manufacturing Software Demos</p><p className="text-xs text-muted-foreground">Enter license key: MANUF-PWA-2026-001</p></div></div>
        ...
        {activated && (
          <div className="mx-4 md:mx-8 p-4 rounded-lg border border-green-500/30 bg-green-500/5 flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-3"><CheckCircle2 className="h-5 w-5 text-green-500" /><div><p className="font-semibold text-sm">Master Copy Ready — All 5 Manufacturing Software Unlocked</p><p className="text-xs text-muted-foreground">Download the complete offline bundle</p></div></div>
        ...
        <SectionHeader icon="🏭" title="Manufacturing & Industrial Production" subtitle="Top 5 Manufacturing Software Clones — Offline Ready." badge="ROW 16" badgeVariant="hot" totalCount={5} />
        <SectionSlider>
          {PRODUCTS.map((p, i) => (
            <div key={p.id} className="min-w-[280px] max-w-[320px] flex-shrink-0 group">
              <Card className="relative overflow-hidden border-border/50 bg-card hover:border-primary/40 transition-all duration-300 hover:scale-[1.05] hover:shadow-[0_0_30px_rgba(249,115,22,0.15)]">
                ...
                <CardContent className="p-4 space-y-3">
                  <Badge variant="outline" className="text-[9px] uppercase tracking-widest text-primary border-primary/30">Manufacturing</Badge>
      ...
      <Dialog open={showKey} onOpenChange={setShowKey}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-4 pt-2">
            <p className="text-sm text-muted-foreground">Enter your license key to unlock all 5 Manufacturing software demos offline.</p>
            <Input placeholder="MANUF-PWA-2026-001" value={keyInput} onChange={e => setKeyInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && activate()} className="font-mono text-center tracking-widest" />
