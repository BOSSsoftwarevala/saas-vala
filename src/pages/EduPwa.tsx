...
const PRODUCTS = [
  {
    id: 'edu-1', name: 'Google Classroom Clone',
    ...
    features: ['Student Management', 'Attendance System', 'Fee Management', 'Exam System', 'Parent Portal', 'Teacher Dashboard'],
    demoFolder: 'google-classroom',
  },
  {
    id: 'edu-2', name: 'Moodle LMS Clone',
    ...
    features: ['Student Management', 'Attendance System', 'Fee Management', 'Exam System', 'Parent Portal', 'Mobile App'],
    demoFolder: 'moodle',
  },
  {
    id: 'edu-3', name: 'Canvas LMS Clone',
    ...
    features: ['Student Management', 'Exam System', 'Fee Management', 'Teacher Dashboard', 'Mobile App', 'Parent Portal'],
    demoFolder: 'canvas',
  },
  {
    id: 'edu-4', name: 'Blackboard Learn Clone',
    ...
    features: ['Attendance System', 'Exam System', 'Fee Management', 'Student Management', 'Teacher Dashboard', 'Mobile App'],
    demoFolder: 'blackboard',
  },
  {
    id: 'edu-5', name: 'Schoology LMS Clone',
    ...
    features: ['Student Management', 'Attendance System', 'Parent Portal', 'Fee Management', 'Exam System', 'Teacher Dashboard'],
    demoFolder: 'schoology',
  },
];
...
function getLicense(): { key: string; activation: string; expiry: string } | null {
  try {
    const raw = localStorage.getItem(`${PFX}-license`);
...
function isLicenseValid(): { valid: boolean; expired: boolean; daysLeft: number } {
  ...
  const now = new Date();
  const exp = new Date(lic.expiry);
  const diff = exp.getTime() - now.getTime();
  const daysLeft = Math.max(0, Math.ceil(diff / (24 * 60 * 60 * 1000)));
  return { valid: diff > 0, expired: diff <= 0, daysLeft };
}
...
function setWishlist(ids: string[]) {
  localStorage.setItem(`${PFX}-wishlist`, JSON.stringify(ids));
}
...
export default function EduPwa() {
  ...
  const handleBuy = () => {
    if (licStatus.valid) {
      toast.success('License active! Use Master Copy to access demos.');
      return;
    }
  ...
  const handleActivate = async () => {
    ...
    if (result.valid) {
      ...
      setKeyInput('');
      toast.success('🎉 License activated for 30 days! All 5 Education demos unlocked.');
    } else {
  ...
  const handleMasterDownload = () => {
    ...
    const bundle = {
      ...
      activated: true,
      license: getLicense(),
      products: PRODUCTS.map(p => ({ name: p.name, repo: p.repo, demoFolder: p.demoFolder, features: p.features })),
      generatedAt: new Date().toISOString(),
    };
    const blob = new Blob([JSON.stringify(bundle, null, 2)], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = 'saas-vala-education-master-copy.json'; a.click();
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <header className="border-b border-border px-4 md:px-8 py-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">Education & EdTech — Offline APK</p>
        </div>
        <div className="flex items-center gap-2">
          {licStatus.valid ? (
            <Badge className="bg-green-500/20 text-green-400 border-green-500/30 gap-1">
              <ShieldCheck className="h-3 w-3" /> {licStatus.daysLeft}d left
            </Badge>
          ) : licStatus.expired ? (
        ...
        {licStatus.expired && (
          <div className="mx-4 md:mx-8 p-4 rounded-lg border border-destructive/30 bg-destructive/5 flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-sm">License Expired</p>
                <p className="text-xs text-muted-foreground">Re-enter a license key to restore 30-day access.</p>
              </div>
            </div>
            <Button size="sm" variant="destructive" onClick={() => setShowActivation(true)} className="gap-1">
              <KeyRound className="h-3 w-3" /> Re-Activate
            </Button>
          </div>
        )}
          ...
          <div className="mx-4 md:mx-8 p-4 rounded-lg border border-primary/30 bg-primary/5 flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">Enter license key: EDU-APK-2026-001</p>
              </div>
            </div>
        ...
        {licStatus.valid && (
          <div className="mx-4 md:mx-8 p-4 rounded-lg border border-green-500/30 bg-green-500/5 flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-sm">Master Copy Ready — {licStatus.daysLeft} days remaining</p>
            ...
            <div key={product.id} className="min-w-[280px] max-w-[320px] flex-shrink-0 group">
              <Card className="relative overflow-hidden border-border/50 bg-card hover:border-primary/40 transition-all duration-300 hover:scale-[1.05] hover:shadow-[0_0_30px_rgba(249,115,22,0.15)]">
                ...
                <CardContent className="p-4 space-y-3">
                  <Badge variant="outline" className="text-[9px] uppercase tracking-widest text-primary border-primary/30">Education</Badge>
                  ...
                  <div className="flex flex-wrap gap-1">
                    ...
                    {product.features.length > 4 && (
                      <Badge variant="secondary" className="text-[8px] px-1.5 py-0 font-medium">+{product.features.length - 4}</Badge>
                    )}
                  </div>
                  ...
                  <div className="flex gap-2 pt-1">
                    <Button size="sm" variant="outline" className="flex-1 text-xs gap-1" onClick={() => window.open(product.repo, '_blank')}>
                  ...
                  {licStatus.valid && (
                    <Button size="sm" variant="outline" className="w-full text-xs gap-1 border-green-500/30 text-green-400 hover:bg-green-500/10" onClick={() => openLocalDemo(product)}>
      ...
      <Dialog open={showActivation} onOpenChange={setShowActivation}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-4 pt-2">
            <p className="text-sm text-muted-foreground">Enter your license key to unlock all 5 Education demos for 30 days.</p>
            <Input
              placeholder="EDU-APK-2026-001"
              value={keyInput}
      ...
      <Dialog open={!!showDemo} onOpenChange={() => setShowDemo(null)}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <FolderOpen className="h-5 w-5 text-green-500" /> {demoProduct?.name} — Local Demo
            </DialogTitle>
          </DialogHeader>
          {demoProduct && (
            <div className="space-y-4 pt-2">
              <div className="rounded-lg border border-border bg-muted/30 p-4">
                ...
                <div className="bg-background rounded p-4 border border-border space-y-3">
                  ...
                  <Badge className="bg-green-500/20 text-green-400 border-green-500/30 text-[10px]">
                    ✅ Running from local /demo folder — 100% Offline
                  </Badge>
                </div>
              </div>
              <Button variant="outline" className="w-full text-xs gap-1" onClick={() => window.open(demoProduct.repo, '_blank')}>
