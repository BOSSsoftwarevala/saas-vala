...
const PRODUCTS = [
  { id: 'social-apk-1', name: 'Facebook Social Network Clone', demoFolder: 'facebook-social', repo: 'https://github.com/saasvala/facebook-social-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Full-featured social network with news feed, messaging and groups.', features: ['User Profiles', 'News Feed', 'Messaging System', 'Likes & Comments', 'Notifications', 'Groups & Communities', 'Media Sharing'] },
  { id: 'social-apk-2', name: 'Instagram Social Media Clone', demoFolder: 'instagram-social', repo: 'https://github.com/saasvala/instagram-social-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Photo and video sharing platform with stories and messaging.', features: ['User Profiles', 'News Feed', 'Messaging System', 'Likes & Comments', 'Notifications', 'Groups & Communities', 'Media Sharing'] },
  { id: 'social-apk-3', name: 'Twitter (X) Microblog Clone', demoFolder: 'twitter-x', repo: 'https://github.com/saasvala/twitter-x-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Microblogging platform with real-time feed and notifications.', features: ['User Profiles', 'News Feed', 'Messaging System', 'Likes & Comments', 'Notifications', 'Groups & Communities', 'Media Sharing'] },
  { id: 'social-apk-4', name: 'LinkedIn Professional Network Clone', demoFolder: 'linkedin-network', repo: 'https://github.com/saasvala/linkedin-network-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Professional networking platform with job listings and messaging.', features: ['User Profiles', 'News Feed', 'Messaging System', 'Likes & Comments', 'Notifications', 'Groups & Communities', 'Media Sharing'] },
  { id: 'social-apk-5', name: 'Reddit Community Platform Clone', demoFolder: 'reddit-community', repo: 'https://github.com/saasvala/reddit-community-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Community-driven discussion platform with subreddits and voting.', features: ['User Profiles', 'News Feed', 'Messaging System', 'Likes & Comments', 'Notifications', 'Groups & Communities', 'Media Sharing'] },
];

const PFX = 'social-pwa';
...
export default function SocialPwa() {
  ...
  const activate = async () => {
    ...
    if (result.valid) {
      ...
      toast.success(`🎉 License activated! Valid for ${daysLeft} days. All 5 Social Media software demos unlocked.`);
  ...
  const handleMasterDownload = () => {
    ...
    const blob = new Blob([JSON.stringify({ bundle: 'SaaS VALA Social Media & Community Platforms Master Copy', version: '2026.1', row: 15, license: lic, products: PRODUCTS.map(p => ({ name: p.name, repo: p.repo, features: p.features, demoFolder: p.demoFolder })), generatedAt: new Date().toISOString() }, null, 2)], { type: 'application/json' });
    const u = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = u; a.download = 'saas-vala-social-media-master-copy.json'; a.click(); URL.revokeObjectURL(u);
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <header className="border-b border-border px-4 md:px-8 py-4 flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-primary">SaaS VALA</h1><p className="text-xs text-muted-foreground">Social Media & Community Platforms — Offline PWA</p></div>
      ...
      <main className="py-6 space-y-6">
        {!licenseState.valid && (
          <div className={cn("mx-4 md:mx-8 p-4 rounded-lg border flex items-center justify-between gap-4 flex-wrap", licenseState.expired ? "border-destructive/30 bg-destructive/5" : "border-primary/30 bg-primary/5")}>
            <div className="flex items-center gap-3">{licenseState.expired ? <AlertTriangle className="h-5 w-5 text-destructive" /> : <Lock className="h-5 w-5 text-primary" />}<div><p className="font-semibold text-sm">{licenseState.expired ? 'License Expired — Enter a new key to continue' : 'Activate to unlock all 5 Social Media Software Demos'}</p><p className="text-xs text-muted-foreground">Enter license key: SOCIAL-APK-2026-001</p></div></div>
        ...
        {licenseState.valid && (
          <div className="mx-4 md:mx-8 p-4 rounded-lg border border-green-500/30 bg-green-500/5 flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-3"><CheckCircle2 className="h-5 w-5 text-green-500" /><div><p className="font-semibold text-sm">Master Copy Ready — All 5 Social Media Software Unlocked</p><p className="text-xs text-muted-foreground">License valid for {licenseState.daysLeft} days · Download the complete offline bundle</p></div></div>
        ...
        <SectionHeader icon="📱" title="Social Media & Community Platforms" subtitle="Top 5 Social Media & Community Software Clones — Offline Ready." badge="ROW 15" badgeVariant="hot" totalCount={5} />
        <SectionSlider>
          {PRODUCTS.map((p, i) => (
            <div key={p.id} className="min-w-[280px] max-w-[320px] flex-shrink-0 group">
              <Card className="relative overflow-hidden border-border/50 bg-card hover:border-primary/40 transition-all duration-300 hover:scale-[1.05] hover:shadow-[0_0_30px_rgba(249,115,22,0.15)]">
                ...
                <CardContent className="p-4 space-y-3">
                  <Badge variant="outline" className="text-[9px] uppercase tracking-widest text-primary border-primary/30">Social Media</Badge>
      ...
      <Dialog open={showKey} onOpenChange={setShowKey}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-4 pt-2">
            <p className="text-sm text-muted-foreground">Enter your license key to unlock all 5 Social Media software demos offline for {LICENSE_DAYS} days.</p>
            <Input placeholder="SOCIAL-APK-2026-001" value={keyInput} onChange={e => setKeyInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && activate()} className="font-mono text-center tracking-widest" />
