...
const PRODUCTS = [
  { id: 'const-pwa-1', name: 'Procore Clone', repo: 'https://github.com/saasvala/procore-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Construction project management with BIM tools and site monitoring.', features: ['Project Management', 'BIM Tools', 'Site Monitoring', 'Cost Estimation', 'Resource Allocation'] },
  { id: 'const-pwa-2', name: 'Buildertrend Clone', repo: 'https://github.com/saasvala/buildertrend-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Project scheduling with cost estimation and contractor management.', features: ['Project Scheduling', 'Cost Estimation', 'Contractor Management', 'Client Dashboard', 'Mobile App'] },
  { id: 'const-pwa-3', name: 'Autodesk Construction Cloud Clone', repo: 'https://github.com/saasvala/autodesk-construction-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'BIM tools with project management and team collaboration.', features: ['BIM Tools', 'Project Management', 'Collaboration', 'Analytics', 'Mobile App'] },
  { id: 'const-pwa-4', name: 'CoConstruct Clone', repo: 'https://github.com/saasvala/coconstruct-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Project management with client communication and budget tracking.', features: ['Project Management', 'Client Communication', 'Budget Tracking', 'Scheduling', 'Mobile App'] },
  { id: 'const-pwa-5', name: 'PlanGrid Clone', repo: 'https://github.com/saasvala/plangrid-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Blueprint management with field collaboration and issue tracking.', features: ['Blueprint Management', 'Field Collaboration', 'Issue Tracking', 'Project Dashboard', 'Mobile App'] },
];

const VALID_KEYS = ['CONST-PWA-2026-001', 'CONST-PWA-2026-002', 'CONST-PWA-2026-003', 'CONST-APK-2026-001'];
const PFX = 'const-pwa';

export default function ConstructionPwa() {
  ...
  const activate = () => {
    if (VALID_KEYS.includes(keyInput.trim().toUpperCase())) {
      ...
      toast.success('🎉 License activated! All 5 Construction & Infrastructure software demos unlocked.');
    } else toast.error('Invalid license key.');
  };

  const download = () => {
    if (!activated) { setShowKey(true); return; }
    const blob = new Blob([JSON.stringify({ bundle: 'SaaS VALA Construction & Infrastructure Master Copy', version: '2026.1', activated: true, products: PRODUCTS.map(p => ({ name: p.name, repo: p.repo, features: p.features })), generatedAt: new Date().toISOString() }, null, 2)], { type: 'application/json' });
    const u = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = u; a.download = 'saas-vala-construction-master-copy.json'; a.click(); URL.revokeObjectURL(u);
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <header className="border-b border-border px-4 md:px-8 py-4 flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-primary">SaaS VALA</h1><p className="text-xs text-muted-foreground">Construction & Infrastructure — Offline PWA</p></div>
      ...
      <main className="py-6 space-y-6">
        {!activated && (
          <div className="mx-4 md:mx-8 p-4 rounded-lg border border-primary/30 bg-primary/5 flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-3"><Lock className="h-5 w-5 text-primary" /><div><p className="font-semibold text-sm">Activate to unlock all 5 Construction & Infrastructure Software Demos</p><p className="text-xs text-muted-foreground">Enter license key: CONST-PWA-2026-001</p></div></div>
        ...
        {activated && (
          <div className="mx-4 md:mx-8 p-4 rounded-lg border border-green-500/30 bg-green-500/5 flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-3"><CheckCircle2 className="h-5 w-5 text-green-500" /><div><p className="font-semibold text-sm">Master Copy Ready — All 5 Construction Software Unlocked</p><p className="text-xs text-muted-foreground">Download the complete offline bundle</p></div></div>
        ...
        <SectionHeader icon="🏗️" title="Construction & Infrastructure" subtitle="Top 5 Construction & Infrastructure Software Clones — Offline Ready." badge="ROW 17" badgeVariant="hot" totalCount={5} />
        <SectionSlider>
          {PRODUCTS.map((p, i) => (
            <div key={p.id} className="min-w-[280px] max-w-[320px] flex-shrink-0 group">
              <Card className="relative overflow-hidden border-border/50 bg-card hover:border-primary/40 transition-all duration-300 hover:scale-[1.05] hover:shadow-[0_0_30px_rgba(249,115,22,0.15)]">
                ...
                <div className="h-32 bg-gradient-to-br from-primary/20 via-primary/5 to-transparent flex items-center justify-center"><div className="w-16 h-16 rounded-xl bg-primary/20 border border-primary/30 flex items-center justify-center text-3xl">🏗️</div></div>
                <CardContent className="p-4 space-y-3">
                  <Badge variant="outline" className="text-[9px] uppercase tracking-widest text-primary border-primary/30">Construction</Badge>
      ...
      <Dialog open={showKey} onOpenChange={setShowKey}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-4 pt-2">
            <p className="text-sm text-muted-foreground">Enter your license key to unlock all 5 Construction & Infrastructure software demos offline.</p>
            <Input placeholder="CONST-PWA-2026-001" value={keyInput} onChange={e => setKeyInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && activate()} className="font-mono text-center tracking-widest" />
