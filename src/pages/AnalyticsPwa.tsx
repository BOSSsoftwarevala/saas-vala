...
const PRODUCTS = [
  { id: 'analytics-apk-1', name: 'Google Analytics Dashboard Clone', demoFolder: 'google-analytics', repo: 'https://github.com/saasvala/google-analytics-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Web analytics platform for tracking and analyzing website traffic with real-time data.', features: ['Data Dashboards', 'Visual Analytics', 'Business Reports', 'Data Integration', 'Real-Time Metrics', 'Custom Charts', 'Data Export'] },
  { id: 'analytics-apk-2', name: 'Tableau BI Dashboard Clone', demoFolder: 'tableau-bi', repo: 'https://github.com/saasvala/tableau-bi-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Powerful business intelligence platform for visualizing and analyzing data.', features: ['Data Dashboards', 'Visual Analytics', 'Business Reports', 'Data Integration', 'Real-Time Metrics', 'Custom Charts', 'Data Export'] },
  { id: 'analytics-apk-3', name: 'Power BI Analytics Clone', demoFolder: 'powerbi-analytics', repo: 'https://github.com/saasvala/powerbi-analytics-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Business analytics platform used to create reports and dashboards.', features: ['Data Dashboards', 'Visual Analytics', 'Business Reports', 'Data Integration', 'Real-Time Metrics', 'Custom Charts', 'Data Export'] },
  { id: 'analytics-apk-4', name: 'Mixpanel Product Analytics Clone', demoFolder: 'mixpanel-analytics', repo: 'https://github.com/saasvala/mixpanel-analytics-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Product analytics platform for tracking user interactions and behaviors.', features: ['Data Dashboards', 'Visual Analytics', 'Business Reports', 'Data Integration', 'Real-Time Metrics', 'Custom Charts', 'Data Export'] },
  { id: 'analytics-apk-5', name: 'Metabase Analytics Platform Clone', demoFolder: 'metabase-analytics', repo: 'https://github.com/saasvala/metabase-analytics-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Open-source business intelligence tool for querying and visualizing data.', features: ['Data Dashboards', 'Visual Analytics', 'Business Reports', 'Data Integration', 'Real-Time Metrics', 'Custom Charts', 'Data Export'] },
];

const PFX = 'analytics-pwa';
...
export default function AnalyticsPwa() {
  ...
  const activate = async () => {
    ...
    if (result.valid) {
      ...
      toast.success(`🎉 License activated! Valid for ${daysLeft} days. All 5 Analytics software demos unlocked.`);
  ...
  const handleMasterDownload = () => {
    ...
    const blob = new Blob([JSON.stringify({ bundle: 'SaaS VALA Data Analytics & BI Master Copy', version: '2026.1', row: 20, license: lic, products: PRODUCTS.map(p => ({ name: p.name, repo: p.repo, features: p.features, demoFolder: p.demoFolder })), generatedAt: new Date().toISOString() }, null, 2)], { type: 'application/json' });
    const u = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = u; a.download = 'saas-vala-analytics-bi-master-copy.json'; a.click(); URL.revokeObjectURL(u);
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <header className="border-b border-border px-4 md:px-8 py-4 flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-primary">SaaS VALA</h1><p className="text-xs text-muted-foreground">Data Analytics & Business Intelligence — Offline PWA</p></div>
      ...
      <main className="py-6 space-y-6">
        {!licenseState.valid && (
          <div className={cn("mx-4 md:mx-8 p-4 rounded-lg border flex items-center justify-between gap-4 flex-wrap", licenseState.expired ? "border-destructive/30 bg-destructive/5" : "border-primary/30 bg-primary/5")}>
            <div className="flex items-center gap-3">{licenseState.expired ? <AlertTriangle className="h-5 w-5 text-destructive" /> : <Lock className="h-5 w-5 text-primary" />}<div><p className="font-semibold text-sm">{licenseState.expired ? 'License Expired — Enter a new key to continue' : 'Activate to unlock all 5 Analytics & BI Software Demos'}</p><p className="text-xs text-muted-foreground">Enter license key: ANALYTICS-APK-2026-001</p></div></div>
        ...
        <SectionHeader icon="📊" title="Data Analytics & Business Intelligence" subtitle="Top 5 Analytics & BI Software Clones — Offline Ready." badge="ROW 20" badgeVariant="hot" totalCount={5} />
        <SectionSlider>
          {PRODUCTS.map((p, i) => (
            <div key={p.id} className="min-w-[280px] max-w-[320px] flex-shrink-0 group">
              <Card className="relative overflow-hidden border-border/50 bg-card hover:border-primary/40 transition-all duration-300 hover:scale-[1.05] hover:shadow-[0_0_30px_rgba(249,115,22,0.15)]">
                ...
                <CardContent className="p-4 space-y-3">
                  <Badge variant="outline" className="text-[9px] uppercase tracking-widest text-primary border-primary/30">Analytics</Badge>
      ...
      <Dialog open={showKey} onOpenChange={setShowKey}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-4 pt-2">
            <p className="text-sm text-muted-foreground">Enter your license key to unlock all 5 Analytics & BI software demos offline for {LICENSE_DAYS} days.</p>
            <Input placeholder="ANALYTICS-APK-2026-001" value={keyInput} onChange={e => setKeyInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && activate()} className="font-mono text-center tracking-widest" />
