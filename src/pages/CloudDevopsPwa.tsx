...
const PRODUCTS = [
  { id: 'cloud-pwa-1', name: 'AWS Management Clone', repo: 'https://github.com/saasvala/aws-management-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Cloud deployment with monitoring, alerts and CI/CD pipelines.', features: ['Cloud Deployment', 'Monitoring & Alerts', 'Analytics Dashboard', 'Mobile App', 'CI/CD Pipelines'] },
  { id: 'cloud-pwa-2', name: 'Azure DevOps Clone', repo: 'https://github.com/saasvala/azure-devops-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'CI/CD pipelines with infrastructure management and monitoring.', features: ['CI/CD Pipelines', 'Infrastructure Management', 'Monitoring Dashboard', 'Mobile App', 'Analytics'] },
  { id: 'cloud-pwa-3', name: 'Jenkins Pipeline Clone', repo: 'https://github.com/saasvala/jenkins-pipeline-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Continuous integration with pipeline automation and analytics.', features: ['Continuous Integration', 'Pipeline Automation', 'Analytics Dashboard', 'Mobile App', 'Alerts'] },
  { id: 'cloud-pwa-4', name: 'Docker Management Clone', repo: 'https://github.com/saasvala/docker-management-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Container management with deployment automation and monitoring.', features: ['Container Management', 'Deployment Automation', 'Mobile App', 'Monitoring Dashboard', 'Analytics'] },
...
export default function CloudDevopsPwa() {
  ...
  const download = () => {
    ...
    const blob = new Blob([JSON.stringify({ bundle: 'SaaS VALA Cloud Computing & DevOps Master Copy', version: '2026.1', activated: true, products: PRODUCTS.map(p => ({ name: p.name, repo: p.repo, features: p.features })), generatedAt: new Date().toISOString() }, null, 2)], { type: 'application/json' });
    const u = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = u; a.download = 'saas-vala-cloud-devops-master-copy.json'; a.click(); URL.revokeObjectURL(u);
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <header className="border-b border-border px-4 md:px-8 py-4 flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-primary">SaaS VALA</h1><p className="text-xs text-muted-foreground">Cloud Computing & DevOps — Offline PWA</p></div>
      ...
      <main className="py-6 space-y-6">
        {!activated && (
          <div className="mx-4 md:mx-8 p-4 rounded-lg border border-primary/30 bg-primary/5 flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-3"><Lock className="h-5 w-5 text-primary" /><div><p className="font-semibold text-sm">Activate to unlock all 5 Cloud & DevOps Software Demos</p><p className="text-xs text-muted-foreground">Enter license key: CLOUD-PWA-2026-001</p></div></div>
        ...
        {activated && (
          <div className="mx-4 md:mx-8 p-4 rounded-lg border border-green-500/30 bg-green-500/5 flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-3"><CheckCircle2 className="h-5 w-5 text-green-500" /><div><p className="font-semibold text-sm">Master Copy Ready — All 5 Cloud & DevOps Software Unlocked</p><p className="text-xs text-muted-foreground">Download the complete offline bundle</p></div></div>
        ...
        <SectionHeader icon="☁️" title="Cloud Computing & DevOps" subtitle="Top 5 Cloud & DevOps Software Clones — Offline Ready." badge="ROW 23" badgeVariant="hot" totalCount={5} />
        <SectionSlider>
          {PRODUCTS.map((p, i) => (
            <div key={p.id} className="min-w-[280px] max-w-[320px] flex-shrink-0 group">
              <Card className="relative overflow-hidden border-border/50 bg-card hover:border-primary/40 transition-all duration-300 hover:scale-[1.05] hover:shadow-[0_0_30px_rgba(249,115,22,0.15)]">
                ...
                <CardContent className="p-4 space-y-3">
                  <Badge variant="outline" className="text-[9px] uppercase tracking-widest text-primary border-primary/30">Cloud & DevOps</Badge>
      ...
      <Dialog open={showKey} onOpenChange={setShowKey}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-4 pt-2">
            <p className="text-sm text-muted-foreground">Enter your license key to unlock all 5 Cloud & DevOps software demos offline.</p>
            <Input placeholder="CLOUD-PWA-2026-001" value={keyInput} onChange={e => setKeyInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && activate()} className="font-mono text-center tracking-widest" />
