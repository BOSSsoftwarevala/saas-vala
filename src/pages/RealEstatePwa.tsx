...
const PFX = 'realestate-pwa';

const PRODUCTS = [
  {
    id: 're-1', name: 'Zillow Real Estate Clone',
    repo: 'https://github.com/saasvala/zillow-realestate-clone-software',
    price: 5, old_price: 10, rating: 4.9,
    description: 'Full-featured real estate marketplace with property listings and agent tools.',
    features: ['Property Listings', 'Agent Dashboard', 'Lead Management', 'Property Search Filters', 'Map Integration', 'Property Analytics', 'Buyer & Seller Accounts'],
    demoFolder: 'zillow-realestate',
  },
  {
    id: 're-2', name: 'Realtor.com Clone',
    repo: 'https://github.com/saasvala/realtor-realestate-clone-software',
    price: 5, old_price: 10, rating: 4.9,
    description: 'Property search platform with agent profiles, listings, and analytics.',
    features: ['Property Listings', 'Agent Dashboard', 'Lead Management', 'Property Search Filters', 'Map Integration', 'Property Analytics', 'Buyer & Seller Accounts'],
    demoFolder: 'realtor-realestate',
  },
  {
    id: 're-3', name: 'MagicBricks Clone',
    repo: 'https://github.com/saasvala/magicbricks-realestate-clone-software',
    price: 5, old_price: 10, rating: 4.9,
    description: 'Property portal for buying, selling, and renting residential & commercial spaces.',
    features: ['Property Listings', 'Agent Dashboard', 'Lead Management', 'Property Search Filters', 'Map Integration', 'Property Analytics', 'Buyer & Seller Accounts'],
    demoFolder: 'magicbricks-realestate',
  },
  {
    id: 're-4', name: '99acres Real Estate Clone',
    repo: 'https://github.com/saasvala/99acres-realestate-clone-software',
    price: 5, old_price: 10, rating: 4.9,
    description: 'Real estate classifieds platform with property search and agent management.',
    features: ['Property Listings', 'Agent Dashboard', 'Lead Management', 'Property Search Filters', 'Map Integration', 'Property Analytics', 'Buyer & Seller Accounts'],
    demoFolder: '99acres-realestate',
  },
  {
    id: 're-5', name: 'Redfin Real Estate Clone',
    repo: 'https://github.com/saasvala/redfin-realestate-clone-software',
    price: 5, old_price: 10, rating: 4.9,
    description: 'Technology-powered real estate brokerage with map-based property search.',
    features: ['Property Listings', 'Agent Dashboard', 'Lead Management', 'Property Search Filters', 'Map Integration', 'Property Analytics', 'Buyer & Seller Accounts'],
    demoFolder: 'redfin-realestate',
  },
];
...
export default function RealEstatePwa() {
  ...
  const handleActivate = async () => {
    ...
    if (result.valid) {
      ...
      toast.success('🎉 License activated for 30 days! All 5 Real Estate demos unlocked.');
  ...
  const handleMasterDownload = () => {
    ...
    const blob = new Blob([JSON.stringify({ bundle: 'SaaS VALA Real Estate Master Copy', version: '2026.1', license: getLicense(), products: PRODUCTS.map(p => ({ name: p.name, repo: p.repo, demoFolder: p.demoFolder, features: p.features })), generatedAt: new Date().toISOString() }, null, 2)], { type: 'application/json' });
    const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'saas-vala-realestate-master-copy.json'; a.click(); URL.revokeObjectURL(url);
    ...
    <div className="min-h-screen bg-background text-foreground">
      <header className="border-b border-border px-4 md:px-8 py-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">Real Estate & Property — Offline APK</p>
        </div>
      ...
      <main className="py-6 space-y-6">
        ...
        {!licStatus.valid && !licStatus.expired && (
          <div className="mx-4 md:mx-8 p-4 rounded-lg border border-primary/30 bg-primary/5 flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-3"><Lock className="h-5 w-5 text-primary" /><div><p className="font-semibold text-sm">Activate to unlock all 5 Real Estate Software Demos</p><p className="text-xs text-muted-foreground">Enter license key: REALESTATE-APK-2026-001</p></div></div>
      ...
      <Dialog open={showActivation} onOpenChange={setShowActivation}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-4 pt-2">
            <p className="text-sm text-muted-foreground">Enter your license key to unlock all 5 Real Estate demos for 30 days.</p>
            <Input placeholder="REALESTATE-APK-2026-001" value={keyInput} onChange={e => setKeyInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleActivate()} className="font-mono text-center tracking-widest" />
