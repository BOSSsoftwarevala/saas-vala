...
const PRODUCTS = [
  { id: 're-pwa-1', name: 'Zillow Property Marketplace Clone', repo: 'https://github.com/saasvala/zillow-property-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Real estate marketplace for buying, selling, and renting properties.', features: ['Property Listings', 'Advanced Property Search', 'Agent Profiles', 'Map Integration', 'Property Inquiry System'] },
  { id: 're-pwa-2', name: 'Realtor.com Property Listing Clone', repo: 'https://github.com/saasvala/realtor-property-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Property listing platform helping buyers and sellers connect with agents.', features: ['Property Listings', 'Agent Profiles', 'Property Search', 'Inquiry Management', 'Listing Dashboard'] },
  { id: 're-pwa-3', name: 'MagicBricks Real Estate Clone', repo: 'https://github.com/saasvala/magicbricks-realestate-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Indian property marketplace for residential and commercial properties.', features: ['Property Listings', 'Property Search', 'Agent Dashboard', 'Property Inquiry', 'Map Integration'] },
  { id: 're-pwa-4', name: '99acres Property Marketplace Clone', repo: 'https://github.com/saasvala/99acres-property-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Online property marketplace connecting buyers, sellers, and agents.', features: ['Property Listings', 'Property Filters', 'Agent Profiles', 'Inquiry System', 'Listing Dashboard'] },
  { id: 're-pwa-5', name: 'Redfin Real Estate Platform Clone', repo: 'https://github.com/saasvala/redfin-realestate-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Real estate platform providing home listings, tours, and agent services.', features: ['Property Listings', 'Virtual Tours', 'Map Integration', 'Agent Profiles', 'Inquiry Management'] },
];

const VALID_KEYS = ['RE-PWA-2026-001', 'RE-PWA-2026-002', 'RE-PWA-2026-003', 'RE-APK-2026-001'];
const PFX = 're-pwa';

export default function RealEstatePwa() {
  ...
  const activate = () => {
    if (VALID_KEYS.includes(keyInput.trim().toUpperCase())) {
      ...
      toast.success('🎉 License activated! All 5 Real Estate software demos unlocked.');
    } else toast.error('Invalid license key.');
  };

  const download = () => {
    if (!activated) { setShowKey(true); return; }
    const blob = new Blob([JSON.stringify({ bundle: 'SaaS VALA Real Estate Master Copy', version: '2026.1', activated: true, products: PRODUCTS.map(p => ({ name: p.name, repo: p.repo, features: p.features })), generatedAt: new Date().toISOString() }, null, 2)], { type: 'application/json' });
    const u = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = u; a.download = 'saas-vala-realestate-master-copy.json'; a.click(); URL.revokeObjectURL(u);
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <header className="border-b border-border px-4 md:px-8 py-4 flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-primary">SaaS VALA</h1><p className="text-xs text-muted-foreground">Real Estate & Property — Offline PWA</p></div>
      ...
      <main className="py-6 space-y-6">
        {!activated && (
          <div className="mx-4 md:mx-8 p-4 rounded-lg border border-primary/30 bg-primary/5 flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-3"><Lock className="h-5 w-5 text-primary" /><div><p className="font-semibold text-sm">Activate to unlock all 5 Real Estate Software Demos</p><p className="text-xs text-muted-foreground">Enter license key: RE-PWA-2026-001</p></div></div>
        ...
        {activated && (
          <div className="mx-4 md:mx-8 p-4 rounded-lg border border-green-500/30 bg-green-500/5 flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-3"><CheckCircle2 className="h-5 w-5 text-green-500" /><div><p className="font-semibold text-sm">Master Copy Ready — All 5 Real Estate Software Unlocked</p><p className="text-xs text-muted-foreground">Download the complete offline bundle</p></div></div>
        ...
        <SectionHeader icon="🏠" title="Real Estate & Property Services" subtitle="Top 5 Real Estate Software Clones — Offline Ready." badge="ROW 07" badgeVariant="hot" totalCount={5} />
        <SectionSlider>
          {PRODUCTS.map((p, i) => (
            <div key={p.id} className="min-w-[280px] max-w-[320px] flex-shrink-0 group">
              <Card className="relative overflow-hidden border-border/50 bg-card hover:border-primary/40 transition-all duration-300 hover:scale-[1.05] hover:shadow-[0_0_30px_rgba(249,115,22,0.15)]">
                ...
                <CardContent className="p-4 space-y-3">
                  <Badge variant="outline" className="text-[9px] uppercase tracking-widest text-primary border-primary/30">Real Estate</Badge>
      ...
      <Dialog open={showKey} onOpenChange={setShowKey}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-4 pt-2">
            <p className="text-sm text-muted-foreground">Enter your license key to unlock all 5 Real Estate software demos offline.</p>
            <Input placeholder="RE-PWA-2026-001" value={keyInput} onChange={e => setKeyInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && activate()} className="font-mono text-center tracking-widest" />
