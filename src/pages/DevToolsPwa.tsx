...
const PRODUCTS = [
  { id: 'devtools-apk-1', name: 'GitHub Developer Platform Clone', demoFolder: 'github-dev', repo: 'https://github.com/saasvala/github-dev-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Code repository and collaboration platform with CI/CD pipelines.', features: ['Code Repository', 'CI/CD Pipeline', 'Issue Tracking', 'Team Collaboration', 'Deployment Automation', 'Code Reviews', 'Dev Dashboard'] },
  { id: 'devtools-apk-2', name: 'GitLab DevOps Platform Clone', demoFolder: 'gitlab-devops', repo: 'https://github.com/saasvala/gitlab-devops-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Complete DevOps platform with CI/CD and issue tracking.', features: ['Code Repository', 'CI/CD Pipeline', 'Issue Tracking', 'Team Collaboration', 'Deployment Automation', 'Code Reviews', 'Dev Dashboard'] },
  { id: 'devtools-apk-3', name: 'Bitbucket Code Repository Clone', demoFolder: 'bitbucket-dev', repo: 'https://github.com/saasvala/bitbucket-dev-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Code repository with pull requests and team collaboration.', features: ['Code Repository', 'CI/CD Pipeline', 'Issue Tracking', 'Team Collaboration', 'Deployment Automation', 'Code Reviews', 'Dev Dashboard'] },
  { id: 'devtools-apk-4', name: 'Jenkins CI/CD Automation Clone', demoFolder: 'jenkins-cicd', repo: 'https://github.com/saasvala/jenkins-cicd-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'CI/CD automation server with pipeline management and deployment.', features: ['Code Repository', 'CI/CD Pipeline', 'Issue Tracking', 'Team Collaboration', 'Deployment Automation', 'Code Reviews', 'Dev Dashboard'] },
  { id: 'devtools-apk-5', name: 'Docker Container Platform Clone', demoFolder: 'docker-container', repo: 'https://github.com/saasvala/docker-container-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Container management platform with deployment automation.', features: ['Code Repository', 'CI/CD Pipeline', 'Issue Tracking', 'Team Collaboration', 'Deployment Automation', 'Code Reviews', 'Dev Dashboard'] },
];

const PFX = 'devtools-pwa';
...
export default function DevToolsPwa() {
  ...
  const activate = async () => {
    ...
    if (result.valid) {
      ...
      toast.success(`🎉 License activated! Valid for ${daysLeft} days. All 5 Developer Tools software demos unlocked.`);
  ...
  const handleMasterDownload = () => {
    ...
    const blob = new Blob([JSON.stringify({ bundle: 'SaaS VALA Developer Tools & DevOps Platforms Master Copy', version: '2026.1', row: 17, license: lic, products: PRODUCTS.map(p => ({ name: p.name, repo: p.repo, features: p.features, demoFolder: p.demoFolder })), generatedAt: new Date().toISOString() }, null, 2)], { type: 'application/json' });
    const u = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = u; a.download = 'saas-vala-devtools-master-copy.json'; a.click(); URL.revokeObjectURL(u);
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <header className="border-b border-border px-4 md:px-8 py-4 flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-primary">SaaS VALA</h1><p className="text-xs text-muted-foreground">Developer Tools & DevOps Platforms — Offline PWA</p></div>
      ...
      <main className="py-6 space-y-6">
        {!licenseState.valid && (
          <div className={cn("mx-4 md:mx-8 p-4 rounded-lg border flex items-center justify-between gap-4 flex-wrap", licenseState.expired ? "border-destructive/30 bg-destructive/5" : "border-primary/30 bg-primary/5")}>
            <div className="flex items-center gap-3">{licenseState.expired ? <AlertTriangle className="h-5 w-5 text-destructive" /> : <Lock className="h-5 w-5 text-primary" />}<div><p className="font-semibold text-sm">{licenseState.expired ? 'License Expired — Enter a new key to continue' : 'Activate to unlock all 5 Developer Tools Software Demos'}</p><p className="text-xs text-muted-foreground">Enter license key: DEVTOOLS-APK-2026-001</p></div></div>
        ...
        {licenseState.valid && (
          <div className="mx-4 md:mx-8 p-4 rounded-lg border border-green-500/30 bg-green-500/5 flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-3"><CheckCircle2 className="h-5 w-5 text-green-500" /><div><p className="font-semibold text-sm">Master Copy Ready — All 5 Developer Tools Software Unlocked</p><p className="text-xs text-muted-foreground">License valid for {licenseState.daysLeft} days · Download the complete offline bundle</p></div></div>
        ...
        <SectionHeader icon="⚙️" title="Developer Tools & DevOps Platforms" subtitle="Top 5 Developer Tools & DevOps Software Clones — Offline Ready." badge="ROW 17" badgeVariant="hot" totalCount={5} />
        <SectionSlider>
          {PRODUCTS.map((p, i) => (
            <div key={p.id} className="min-w-[280px] max-w-[320px] flex-shrink-0 group">
              <Card className="relative overflow-hidden border-border/50 bg-card hover:border-primary/40 transition-all duration-300 hover:scale-[1.05] hover:shadow-[0_0_30px_rgba(249,115,22,0.15)]">
                ...
                <CardContent className="p-4 space-y-3">
                  <Badge variant="outline" className="text-[9px] uppercase tracking-widest text-primary border-primary/30">Developer Tools</Badge>
      ...
      <Dialog open={showKey} onOpenChange={setShowKey}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-4 pt-2">
            <p className="text-sm text-muted-foreground">Enter your license key to unlock all 5 Developer Tools software demos offline for {LICENSE_DAYS} days.</p>
            <Input placeholder="DEVTOOLS-APK-2026-001" value={keyInput} onChange={e => setKeyInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && activate()} className="font-mono text-center tracking-widest" />
